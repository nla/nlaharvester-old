...
public class SimplePostTool {
  ...
  static void warnIfNotExpectedResponse(String actual,String expected) {
    if(actual.indexOf(expected) < 0) {
      logger.warn("Unexpected response from Solr: '" + actual + "' does not contain '" + expected + "'");
    }
  }
  ...
  public void postData(Reader data, Writer output) throws IOException{
    ...
    try {
      ...
      urlc.setConnectTimeout(300000);	//5 min
      urlc.setReadTimeout(300000);		// the most important of the timeouts      
      ...
      try {
        logger.fatal("Solr returned an error: " + urlc.getResponseMessage());
      } catch (IOException f) { }
      logger.fatal("Connection error (is Solr running at " + solrUrl + " ?): " + e);
    } finally {
