...
public class MarcConverter extends GenericStep {
    public String getName() {
        return "MarcConverter";
    }
    
    public Document processRecord(Document record, int record_position) throws Exception {

        harvester.processor.util.MarcConverter converter = new harvester.processor.util.MarcConverter();
        String after_conversion = null;
        converter.initialize(record.asXML());
        ...
        if (converter.isValid()) {
            ...
            logger.logfailedrecord(StepLogger.RECORD_ERROR,"Record has invalid type." , record, position, getName(), stepid,  record_position);
            return null;
        }
        return record;
    }
